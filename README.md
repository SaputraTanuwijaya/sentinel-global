...
> **Status: planned.** Tracked in the project roadmap as Phase 3.

The intended setup is a multi-stage `Dockerfile` (`oven/bun:1` builder, `oven/bun:1-alpine` runner) with non-watch `build` / `start` scripts in `package.json` and a `.dockerignore` excluding `node_modules`, `.git`, `.env`, `.claude/`, `docs/`, and generated bundles. Env vars are passed at runtime via `--env-file .env`.

This README will be updated with the actual `docker build` / `docker run` commands once the Dockerfile lands.

---

## Deployment

> **Status: planned.** Tracked in the project roadmap as Phase 4.

Bun + Elysia is **not Vercel-native** — Vercel's serverless model doesn't fit a persistent process with an in-memory cache + active WebGL context on the client side. The current recommendation is **Fly.io** (free hobby tier, runs the Docker image as-is, supports `fly secrets` for env). Railway, Render, and Koyeb are also viable.

Once a deployment lands, this README will list the live URL and the `fly deploy` / equivalent commands.
